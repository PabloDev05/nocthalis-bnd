 ...
 * ❗ Respeta TU convención:
 *    - Porcentajes en "puntos %" (enteros). Ej: 12 => 12%.
 *    - NO se normaliza a 0..1 aquí (eso lo hace el runner).
 *    - attackSpeed son TICKS (enteros). `attackSpeedPct` (legacy) suma ticks.
 */
...
export type PassiveNumbers = {
  ...
  // Velocidad de ataque: TICKS (enteros, no %)
  attackSpeedFlat?: number;    // +X ticks
  /** Alias legacy: si viene, también suma TICKS (para no romper nada). */
...
export const DEFAULT_PASSIVES: Record<
  ...
  { name: string; description: string; effects: PassiveNumbers }
> = {
  Vampire: {
    name: "Crimson Grace",
  ...
  Werewolf: {
    ...
    effects: { damageReductionPct: 5, attackSpeedFlat: 2 },
  },
  Necromancer: {
 ...
 */
export function applyDefaultPassivesToCombat<T extends CombatWritable>(
  className: string | undefined,
  combatLike: T,
  opts?: { dominant?: DominantFlavor }
): T {
  const out: T = { ...(combatLike ?? ({} as T)) };
  const eff = effectsForClassName(className);
  const dominant: DominantFlavor =
    opts?.dominant ??
    (/\b(necromancer|exorcist|mage|wizard|sorcer(er|ess))\b/i.test(String(className ?? ""))
      ? "magical"
      : "physical");
  ...
  // 3) Velocidad de ataque: TICKS (enteros)
  const asFlat = toInt(eff.attackSpeedFlat, 0) + toInt(eff.attackSpeedPct, 0); // alias legacy suma como ticks
  if (asFlat !== 0) {
    out.attackSpeed = clampMin(toInt(out.attackSpeed, 0) + asFlat, 1);
  }
